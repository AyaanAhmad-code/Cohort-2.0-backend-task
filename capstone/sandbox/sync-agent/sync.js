...
const bucketName = "codespace-bucket-1";
...
async function downloadFileFromS3(s3Objects) {
    ...
    for(const file of s3Objects) {
        ...
        // Stream the file content to local directory
...
async function uploadFileToS3(filePath) {
    try {
        ...
        if(filePath.includes('node_modules') || filePath.includes('.env')) {
            return; // Skip node_modules and .env files
        }
